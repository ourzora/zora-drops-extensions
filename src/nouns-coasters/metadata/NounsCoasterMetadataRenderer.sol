    ...
    MetadataRenderAdminCheck
{
    ...
    function insertLayerAtIndex(
        uint256 index,
        NounsCoasterLayerData calldata layerData
    ) external onlyOwner {
        if (index == 0) {
            dataLayers.push(
                DataLayer({
                    data: sstore2.write(abi.encode(layerData)),
                    count: layerData.property.items.length
                })
            );
        }
        dataLayers[index] = DataLayer({
            data: sstore2.write(abi.encode(layerData)),
            count: layerData.property.items.length
        });
    }
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < 4; ++i) {
                ...
                queryString = string.concat(
                    queryString,
                    "&images=",
                    _getItemImage(item, property.name)
                );
            }
            ...
            for (uint16 i = 0; i < 4; ++i) {
                ...
                uint256 numBodyProperties = _properties[0 + variant]
                    .items
                    .length;
                ...
                queryString = string.concat(
                    queryString,
                    "&images=",
                    _getItemImage(item, _properties[0 + variant].name)
                );

                // ok, bump the seed and move to property 2
                seed >>= 16;

                uint256 numAccProperties = _properties[4 + variant]
                    .items
                    .length;
                ...
                queryString = string.concat(
                    queryString,
                    "&images=",
                    _getItemImage(item, _properties[4 + variant].name)
                );
    ...
    function _getItemImage(Item memory _item, string memory _propertyName)
        private
        view
